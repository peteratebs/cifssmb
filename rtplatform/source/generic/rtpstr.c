...
{
unsigned int len_targ;
unsigned int len_src;

	len_src = rtp_strlen(src);
	len_targ = rtp_strlen(targ);
	rtp_strncpy(targ+len_targ, src, (unsigned int) n);

	if (len_src > (unsigned int) n)
	{
		len_src = (unsigned int)n;
	}
...
long rtp_memcat(char * dst, long offset, const char * src, long size)
{
	if (dst)
	{
		rtp_memcpy((unsigned char*) (dst + offset), (unsigned char*) src, (unsigned int)size);
	}
