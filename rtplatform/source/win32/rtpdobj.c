...
FSOBJ* winDirObj;
    ...
    if (winDirObj->handle == (long)(-1))
    {
        free (winDirObj);
#ifdef RTP_DEBUG
		RTP_DEBUG_OUTPUT_ERRNO("rtp_file_gfirst:");
#endif
        return (-1);
    }
...
int rtp_file_gnext (void * dirobj)
{
    ...
    {
#ifdef RTP_DEBUG
		RTP_DEBUG_OUTPUT_ERRNO("rtp_file_gnext:");
#endif
        return (-1);
    }
...
int rtp_file_get_attrib (void * dirobj, unsigned char * attributes)
{
    ...
    {
#ifdef RTP_DEBUG
		RTP_DEBUG_OUTPUT_ERRNO("rtp_file_get_attrib:");
#endif
        return (-1);
    }
...
unsigned int sizelimit;
    ...
    if (!dirobj)
    {
		...
		RTP_DEBUG_OUTPUT_ERRNO("rtp_file_get_name:");
#endif
        return (-1);
    }
...
#ifdef RTP_DEBUG
        SetLastError (ERROR_INVALID_HANDLE);
		RTP_DEBUG_OUTPUT_ERRNO("rtp_file_get_time:");
#endif
        return (-1);
    }
