...
/* Character to use if Unicode to JIS fails */
#define JISDEFAULTCHAR '_'

/* Character to use if JIS to Unicode lookup fails */
...
/* UCODE, JIS */
const unsigned short RTSMB_FAR rtsmb_ucode2jis[NUMJISCHARS][2] = {
...
unsigned short giJapaneseToUnicode[94][94] =
{
	{ // 0
...
unsigned short giUnicodeToEucJp [UNICODE_MAP_SIZE*2] =
{
	0x00A2, 0xA1F1,
	0x00A3, 0xA1F2,
	0x00A7, 0xA1F8,
...
long w, w1;
    ...
    {
    	u[0] = (unsigned short) (sjisChar[0]);
		return (1);
    }
    else if (sjisChar[0] >= 0xa1 && sjisChar[0] <= 0xdf) /* Katakana */
    {
    	u[0] = (unsigned short)((unsigned short)0xfec0 + (unsigned short) (sjisChar[0]));
		return (1);
    }
...
unsigned short w;
    ...
    {
        u = (unsigned short) (u - (unsigned short)0xfec0);
        pjis[0] = (unsigned char) u;
        return(1);
    }
    ...
    /* Now binary search the rtsmb_ucode2jis look up table */
    low  = 0;
    high = NUMJISCHARS-1;
    ...
    while (low <= high)
    {
        ...
        if (u == rtsmb_ucode2jis[midpoint][0])
        {
            w = rtsmb_ucode2jis[midpoint][1];
			pjis[0] = (unsigned char) (w>>8);
			pjis[1] = (unsigned char) (w&0xff);
			return (2);
        }
